...
//import au.com.darrenyates.datalearner2.ScriptC_rtscript;
...
//import au.com.darrenyates.datalearnerpro.RenderScriptGain;
...
//  private RenderScriptGain gainnew;
	/**
	 * The Tree object
	 */
...
//  RenderScript rs;
//  ScriptC_rtscript script;

//  public RandomTree(Context context) {
//
//    this.context = context;
////    rs = RenderScript.create(context);
////	script = new ScriptC_rtscript(rs);
//  }
...
//    Utils.checkForRemainingOptions(options);
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		if (m_KValue < 1) {
			m_KValue = (int) log2(data.numAttributes() - 1) + 1;
		}
...
//-----------------------------------------------------------------------------------------------------
		...
		//-----------------------------------------------------------------------------------------------------
//    //--------------------------------------------------------------------------------------------------
//    public double entropyConditionedOnRowsRS(double[][] matrix) {
//      double returnValue = 0, sumForRow, total = 0;
////		RenderScript rs = RenderScript.create(context);
////		ScriptC_rtscript script = new ScriptC_rtscript(rs);
//
//      for (int i = 0; i < matrix.length; i++) {
//
//			Float2 values = script.reduce_addDouble(matrix[i]).get();
//
//			returnValue = returnValue + (double) values.x;
////			if (values.y > 0) returnValue = returnValue - ((double) values.y * Math.log((double) values.y));
//			if (values.y > 0) returnValue = returnValue - lnFunc((double) values.y);
//			total += (double) values.y;         //sumForRow
////			System.out.println(" >>> NEW  - returnValue:"+returnValue+"\ttotal:"+total);
//      }
//      if (Utils.eq(total, 0)) {
//        return 0;
//      }
...
////		return (double)((float)(-returnValue / (total * log2)));
//    }
...
//      double one = priorVal - ContingencyTables.entropyConditionedOnRows(dist);
//      long time1 = System.nanoTime() - start;
//      //      System.out.println("ORIG: "+one);
////      double two = priorVal - entropyConditionedOnRowsRS(dist);
//
////      double two = priorVal - MainActivity.entropyConditionedOnRowsRS(dist, context);
//      start = System.nanoTime();
//      double two = priorVal - ContingencyTables.entropyConditionedOnRowsRS(dist, context);
//      long time2 = System.nanoTime()-start;
//
//      System.out.println("                             ORIG: "+one+"\tNEW: "+two+"\t\tDIFF: "+(one-two)+"\t\tContext:"+context.toString());
//      System.out.println("TIME1:"+time1+"\tTIME2:"+time2+"\tDIFF:"+(time2-time1));
//      return(two);
//      System.out.println(context.toString());
//      return priorVal - ContingencyTables.entropyConditionedOnRowsRS(dist, context);
...
//    RenderScript rs = RenderScript.create(context);
//    ScriptC_rtscript treeScript = new ScriptC_rtscript(rs);
//  }
}
