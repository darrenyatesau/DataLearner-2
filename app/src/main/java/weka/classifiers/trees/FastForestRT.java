 ...
 */
public class FastForestRT extends AbstractClassifier implements OptionHandler,
...
//--------------------------------------------------------------------------------
    ...
    protected boolean useSkipping = false;
...
//        int[] attIndicesWindow = new int[data.numAttributes() - 1];
        ...
        if (data.classAttribute().isNumeric()) {
            trainVariance
                    = FastForestRT.singleVariance(classProbs[0], totalSumSquared, totalWeight)
                    / totalWeight;
    ...
    public String toString() {
        ...
        if (m_Tree == null) {
            return "FastForestRT: no model has been built yet.";
        } else {
            return "\nFastForestRT\n==========\n"
                    + m_Tree.toString(0)
                    + "\n"
    ...
    public String graph() throws Exception {
        ...
        if (m_Tree == null) {
            throw new Exception("SSR_RandomTree: No model built yet.");
        }
        StringBuffer resultBuff = new StringBuffer();
        m_Tree.toGraph(resultBuff, 0, null);
        String result
                = "digraph SSR_RandomTree {\n" + "edge [style=bold]\n" + resultBuff.toString()
                + "\n}\n";
        return result;
    }
    ...
    protected class Tree implements Serializable {
        ...
        protected String toString(int level) {
            ...
            try {
                ...
                e.printStackTrace();
                return "SSR_RandomTree: tree can't be printed";
            }
        }
        ...
        protected void backfitData(Instances data, double[] classProbs,
            ...
            if (data.classAttribute().isNumeric()) {
                ...
                priorVar
                        = FastForestRT.singleVariance(totalSum, totalSumSquared,
                                totalSumOfWeights);
            }
...
//--------------------------------------------------------------------------------------------------------------
            if (data.numInstances() < numRecords * 0.125)
                k = (int)Utils.log2((data.numAttributes()) * ((numRecords+0.0)/data.numInstances())) + 1;
            ...
            if (attribute.isNominal()) {
                ...
                for (int i = 0; i < indexOfFirstMissingValue; i++) {
                    ...
                    if (inst.value(att) > currSplit) {
                        currVal
                                = FastForestRT.variance(currSums, currSumSquared, currSumOfWeights);
                        if (currVal < bestVal) {
                            bestVal = currVal;
...
//---------------------------------------------------------------------------------------
    ...
    public static void main(String[] argv) {
        runClassifier(new FastForestRT(), argv);
    }
}
