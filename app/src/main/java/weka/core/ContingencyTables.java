...
//import au.com.darrenyates.datalearnerpro.ScriptC_rtscript;
...
public class ContingencyTables
	...
	public static double entropyConditionedOnRows(double[][] matrix) {
		...
		for (int i = 0; i < matrix.length; i++) {
			...
			total += sumForRow;
//			System.out.println(" >>> ORIG - returnValue:"+returnValue+"\ttotal:"+total);
		}
		if (Utils.eq(total, 0)) {
			return 0;
		}
...
//		return (double)((float)(-returnValue / (total * log2)));
	...
	//--------------------------------------------------------------------------------------------------
//	public static double entropyConditionedOnRowsRS(double[][] matrix, Context context) {
//		double returnValue = 0, sumForRow, total = 0;
//		RenderScript rs = RenderScript.create(context);
//		ScriptC_rtscript script = new ScriptC_rtscript(rs);
////		Allocation input1 = Allocation.createSized(rs, Element.F64(rs), matrix[0].length);
//
//		for (int i = 0; i < matrix.length; i++) {
//
////			input1.copyFrom(matrix[i]);
////			ScriptC_rtscript.result_float2 result2 = script.reduce_addDouble(input1);
////			Float2 values = result2.get();
